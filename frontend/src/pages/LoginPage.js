...
import { Mail, Lock, Eye, EyeOff, ArrowRight, AlertCircle, Sun, Moon } from 'lucide-react';
...
function Spinner() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
      style={{ animation: 'lgn-spin 0.8s linear infinite', flexShrink: 0 }}>
...
export default function LoginPage() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setLoginErr(err.response?.data?.message || 'Email ya password galat hai.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes lgn-up   { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }
        ...
        .lgn-submit:active:not(:disabled) { transform: scale(0.975); }
        .lgn-submit:disabled { opacity: 0.65; cursor: not-allowed; }
        ...
        .lgn-theme-btn:hover { background: var(--bg-alt); color: var(--accent); }

        .lgn-divider {
          display: flex; align-items: center; gap: 12; margin: 22px 0;
        }
        .lgn-divider-line { flex: 1; height: 1px; background: var(--border); }
        .lgn-divider-text { font-size: 11px; color: var(--text-4); font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; }
        ...
        .lgn-secondary-link span { color: var(--accent); font-weight: 700; }
        .lgn-secondary-link:hover { border-color: var(--accent); background: var(--accent-light); }
        ...
        @media (max-width: 480px) {
          .lgn-card { padding: 28px 20px 24px; border-radius: var(--r-lg); }
        }
      `}</style>

      <div className="lgn-page">
        <div className="lgn-card" style={{ position: 'relative' }}>

          {/* Theme toggle */}
          <button className="lgn-theme-btn" onClick={toggleTheme} title={isDark ? 'Light mode' : 'Dark mode'}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-3)', lineHeight: 1.6 }}>
              Apna account access karne ke liye login karo
            </p>
          </div>
            ...
            <Field
              label={
                <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  Password
                  <Link to="/forgot-password" className="lgn-forgot">Bhool gaye?</Link>
                </span>
              }
              icon={Lock}
          ...
          <div className="lgn-divider" style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '22px 0' }}>
            <div className="lgn-divider-line" />
            <span className="lgn-divider-text">ya</span>
            <div className="lgn-divider-line" />
          </div>

          <Link to="/register" className="lgn-secondary-link">
            Naya account? <span>Register karo →</span>
          </Link>
