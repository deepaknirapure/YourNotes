...
const S = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .yn-feature{display:flex;align-items:flex-start;gap:12px;margin-bottom:16px;}
  .yn-feature-icon{width:28px;height:28px;border-radius:6px;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
  ...
  return (
    <div className="yn-auth">
      ...
      <div className="yn-left">
        ...
        <div style={{ position: "absolute", top: "30%", right: -80, width: 280, height: 280, background: "radial-gradient(circle, rgba(229,91,45,.07) 0%, transparent 70%)" }} />
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <div className="yn-badge">
            ...
            <span style={{ fontSize: 10, fontWeight: 700, color: "#E55B2D", letterSpacing: ".08em" }}>FREE FOREVER</span>
          </div>
          <h2 className="yn-h1">Start your<br />learning journey.</h2>
          <p className="yn-sub" style={{ marginBottom: 32 }}>Notes, AI summaries, flashcards aur community — sab kuch free.</p>
          {[
            { color: "#4F46E5", bg: "rgba(79,70,229,.12)", label: "Smart Notes", desc: "Rich text editor with auto-save" },
            { color: "#E55B2D", bg: "rgba(229,91,45,.12)", label: "AI Powered", desc: "Summaries & flashcards in seconds" },
            { color: "#10b981", bg: "rgba(16,185,129,.12)", label: "Community", desc: "Share notes with other students" },
          ].map((f, i) => (
            <div key={i} className="yn-feature">
              <div className="yn-feature-icon" style={{ background: f.bg }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "rgba(255,255,255,.35)" }}>{f.desc}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="yn-right">
        <div style={{ marginBottom: 36 }}>
          <h2 style={{ fontSize: 28, fontWeight: 800, color: "#fff", letterSpacing: "-1.2px", marginBottom: 6 }}>Create Account</h2>
          <p style={{ color: "rgba(255,255,255,.35)", fontSize: 14 }}>Apna free account banayein.</p>
        </div>
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <FieldGroup label="Full Name" icon={<User size={14} />} type="text" placeholder="Aapka naam"
            value={form.name} onChange={v => setForm({ ...form, name: v })} />
          <FieldGroup label="Email Address" icon={<Mail size={14} />} type="email" placeholder="name@example.com"
            value={form.email} onChange={v => setForm({ ...form, email: v })} />
          <FieldGroup label="Password" icon={<Lock size={14} />} type="password" placeholder="Min. 6 characters"
            value={form.password} onChange={v => setForm({ ...form, password: v })} />
          <button type="submit" disabled={loading} className="yn-btn" style={{ marginTop: 8 }}>
        ...
        <p style={{ textAlign: "center", marginTop: 28, fontSize: 13, color: "rgba(255,255,255,.3)" }}>
          ...
          <Link to="/login" style={{ color: "#E55B2D", fontWeight: 700 }}>Sign in karein</Link>
        </p>
