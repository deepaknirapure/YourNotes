...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pg-menu-btn { display: none; background: #1e1b1b; border: 1px solid #2a2525; border-radius: 10px; cursor: pointer; padding: 10px; color: #000; }
  ...
  .cm-tabs { 
    display: flex; gap: 6px; background: #2a2525; border-radius: 14px; padding: 5px; 
  }
  .cm-tab { 
    padding: 10px 18px; border: none; border-radius: 10px; font-size: 11px; font-weight: 900; 
    cursor: pointer; transition: 0.3s; display: flex; align-items: center; gap: 8px; color: #8a7f7f; background: transparent; letter-spacing: 0.5px;
  }
  .cm-tab.active { background: #000000; color: #ff5734; }
  
  .cm-search-wrap { 
    position: relative; display: flex; align-items: center; background: #1e1b1b; 
    border: 1px solid #2a2525; border-radius: 14px; padding: 0 16px; 
    width: 320px; transition: 0.3s;
  }
  .cm-search-wrap:focus-within { border-color: #000; background: #1e1b1b; box-shadow: 0 0 15px rgba(0,0,0,0.05); }
  .cm-search { 
  ...
  .btn-neon {
    background: #000000; color: #ff5734; border: none; border-radius: 14px; padding: 12px 20px; 
  ...
  .cm-card:hover { border-color: #ff5734; transform: translateY(-5px); box-shadow: 0 10px 30px rgba(0,0,0,0.03); }
  
  .cm-card-header { display: flex; align-items: center; gap: 14px; margin-bottom: 20px; }
  .cm-author-avatar { 
    width: 44px; height: 44px; border-radius: 14px; background: #ff5734; 
    display: flex; align-items: center; justify-content: center; 
    font-size: 18px; font-weight: 900; color: #000; flex-shrink: 0; 
  }
  .cm-author-name { font-size: 15px; font-weight: 800; color: #000; }
  .cm-date { font-size: 12px; color: #8a7f7f; font-weight: 700; text-transform: uppercase; }
  
  .cm-card-title { font-size: 18px; font-weight: 900; color: #000; margin-bottom: 10px; line-height: 1.3; }
  .cm-card-preview { 
    font-size: 14px; color: #8a7f7f; display: -webkit-box; -webkit-line-clamp: 2; 
    -webkit-box-orient: vertical; overflow: hidden; line-height: 1.7; margin-bottom: 24px; flex: 1;
  }
  
  .cm-card-footer { display: flex; align-items: center; justify-content: space-between; border-top: 1px solid #1e1b1b; padding-top: 20px; }
  .cm-stat-item { display: flex; align-items: center; gap: 8px; font-size: 13px; color: #000; font-weight: 800; background: #1e1b1b; padding: 6px 12px; border-radius: 10px; border: none; cursor: pointer; transition: 0.2s; }
  .cm-stat-item:hover { background: #ff5734; }
  
  .cm-action-btn { 
    display: flex; align-items: center; gap: 8px; background: #151313; color: #ff5734; 
  ...
  .pg-spinner { width: 30px; height: 30px; border: 3px solid #2a2525; border-top-color: #ff5734; border-radius: 50%; animation: spin .8s linear infinite; }
    ...
    <div className="pg-wrap">
      ...
      <div className="pg-main">
        <div className="pg-topbar">
          ...
          <div style={{ padding: "8px", borderRadius: "10px", background: "#000" }}><Users size={18} color="#ff5734" /></div>
          <span style={{ fontSize: 20, fontWeight: 900, letterSpacing: "-0.5px" }}>COMMUNITY <span style={{color: "#ff5734", background: "#000", padding: "2px 6px", borderRadius: "4px"}}>HUB</span></span>
        </div>
              ...
              {subjects.map(({ subject, items }) => (
                <section key={subject}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 900, background: "#000", color: "#ff5734", padding: "3px 10px", borderRadius: "6px" }}>{items.length} FILES</span>
                  </div>
                        ...
                        <div className="cm-card-footer">
                          <div style={{ display: "flex", gap: 8 }}>
                            <button className="cm-stat-item" onClick={() => toggleLike(note._id)}>
                              <Heart size={15} fill={note.liked ? "#000" : "none"} color={note.liked ? "#ff5734" : "#000"} />
                              {note.likesCount || 0}
                            </button>
                          ...
                          <div style={{ display: "flex", gap: 10 }}>
                            <button className="cm-stat-item" style={{background: note.saved ? "#ff5734" : "#1e1b1b"}} title="Save to Hub">
                                <Bookmark size={15} />
                            </button>
